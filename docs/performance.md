...
| a take with the colour camera in it | 21.4 MB/s with colour at 30 fps and 18.2 MB/s at 15 fps in dim light, against 14.9 MB/s without: 77, 65 and 54 GB an hour | derived rather than measured on a sensor. `FRAME_BYTES`, 486 KiB of depth and registered colour a frame, was measured on the sensor; `COLOUR_FRAME_BYTES`, 215,082 bytes, is the size one real 1920x1080 colour frame came to, which the MJPEG backpressure measurement synthesised its frames at. Which frame, scene and light that was is not recorded, and a JPEG's size follows the scene. The number that settles it is `/record/state`'s `storage.bytesPerSec` during a take on the sensor, read after a five-second window has closed inside the take |
| the take buffer against a stalled card | 64 MiB holds 3.1 s at 21.4 MB/s, against 4.5 s at 14.9 MB/s | arithmetic on the row above. Past it the recorder drops frames and colour messages alike and writes the count of dropped frames into the take's marks log |
...
| the colour camera's encode on the capture node while a take records | every take shot with colour on runs the encoder, 5.50 ms a frame on its own thread on an M2 Max, and the Pi 5 has not run it under a take | `grabber --profile` on the Pi with colour on and a take recording, read by `prof-summary`, holding 30.0 delivered fps |
| the capture node's card at the colour take rate | no take on the Pi has been written at 21.4 MB/s | a take of several minutes on the Pi with colour on, reading `dropped` off the take's marks log |
...
Five, all carried in the vendored source and all pinned by `tools/vendor-check.mjs`. `third_party/UPSTREAM.md` carries each in full with the interleaved A/B
behind its number.
...
| declare `ColorDecoder`, `defaultColorDecoder` and a decoder overload per pipeline | no number: the header adds a way to ask for a decoder and does no work of its own. Its one cost is that the scoped enum makes `-DENABLE_CXX11=ON` a requirement of this fork | nothing to time. `tools/cpp-check.mjs` compiles the grabber in eight configurations of the pipeline and colour-decoder macros, and one probe per decoder holds that its enumerator exists only on a build carrying it |
| pick the colour decoder by name, prefer a decoder that holds no device, and never substitute | not measured: no colour-decode timing exists on any machine here. What the order buys is reach: on a build carrying TurboJPEG the default never reaches VAAPI, and VAAPI on an AMD RX 9070 XT under Mesa 26 initialises, decodes about 150 frames, then loses the amdgpu context and aborts the process | a number would come from interleaved `--color-decoder` arms on one machine, read as delivered colour frames per second off the periodic log. `grabber --profile` cannot see it: decode runs on libfreenect2's rgb thread and `--profile` times the serial half of the frame loop. `tools/decoder-check.mjs` holds that each enumerator builds its own processor in the loaded library |
