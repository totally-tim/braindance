...
| declare `ColorDecoder`, `defaultColorDecoder` and a decoder overload per pipeline | no number: the header adds a way to ask for a decoder and does no work of its own. Its one cost is that the scoped enum makes `-DENABLE_CXX11=ON` a requirement of this fork | nothing to time. `tools/cpp-check.mjs` compiles the grabber in eight configurations of the pipeline and colour-decoder macros and holds that an enumerator exists only where its decoder does |
| pick the colour decoder by name, prefer software decode, and never substitute | not measured: no colour-decode timing exists on any machine here. What the order buys is reach: on a build carrying TurboJPEG the default never reaches VAAPI, and VAAPI on an AMD RX 9070 XT under Mesa 26 initialises, decodes about 150 frames, then loses the amdgpu context and aborts the process | a number would come from interleaved `--color-decoder` arms on one machine, read as delivered colour frames per second off the periodic log. `grabber --profile` cannot see it: decode runs on libfreenect2's rgb thread and `--profile` times the serial half of the frame loop |
